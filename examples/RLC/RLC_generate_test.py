...
if __name__ == '__main__':
    ...
    X = np.hstack((t_sim.reshape(-1, 1), x1, u.reshape(-1, 1)))
    COL_T = ['time']
    COL_X = ['V_C', 'I_L']
    COL_U = ['V_IN']
    COL = COL_T + COL_X + COL_U
    df_X = pd.DataFrame(X, columns=COL)
    df_X.to_csv(os.path.join("data", "RLC_data_test_lin.csv"), index=False)

    X = np.hstack((t_sim.reshape(-1, 1), x2, u.reshape(-1, 1)))
    COL_T = ['time']
    COL_X = ['V_C', 'I_L']
    COL_U = ['V_IN']
    COL = COL_T + COL_X + COL_U
    df_X = pd.DataFrame(X, columns=COL)
    df_X.to_csv(os.path.join("data", "RLC_data_test_nl.csv"), index=False)
